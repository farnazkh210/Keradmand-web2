...
class ZohoCreatorHandler
{
            ...
            if (!is_wp_error($fieldsMetaResponse)) {
                ...
                if (count($fields) > 0) {
                    ...
                    foreach ($fields as $field) {
                        if (isset($field->is_lookup_field) && $field->is_lookup_field) continue;
                        if (in_array($field->type, [9, 20, 21, 22, 23, 24, 26, 28, 31, 35, 36, 37, 38, 39])) continue;
                        ...
                        if (in_array($field->type, [18, 19, 25, 32, 33])) {
                            ...
                            if ($field->mandatory) $response['requiredFileUploadFields'][] = $field->link_name;
                        } else {
                            if (isset($field->subfields)) {
                                foreach ($field->subfields as $subfield) {
                                    if (!$subfield->is_hidden) {
                                        ...
                                        if ($field->mandatory) $response['required'][] = $subfield->link_name;
                                    }
                                }
                            } else {
                                ...
                                if ($field->mandatory) $response['required'][] = $field->link_name;
                            }
                        }
                    }
                }
    ...
    public function execute(IntegrationHandler $integrationHandler, $integrationData, $fieldValues, $entryID, $logID)
    {
        ...
        if (
            ...
            return new WP_Error('tokenDetails error', __('tokenDetails error', 'bitform'));
        }
