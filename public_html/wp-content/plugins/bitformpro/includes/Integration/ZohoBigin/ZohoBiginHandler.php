...
class ZohoBiginHandler
{
    ...
    {
        add_action('wp_ajax_bitforms_zbigin_generate_token', array(__CLASS__, 'generateTokens'));
        add_action('wp_ajax_bitforms_zbigin_refresh_modules', array(__CLASS__, 'refreshModulesAjaxHelper'));
        add_action('wp_ajax_bitforms_zbigin_refresh_notetypes', array(__CLASS__, 'refreshNoteTypesAjaxHelper'));
        add_action('wp_ajax_bitforms_zbigin_refresh_related_lists', array(__CLASS__, 'refreshRelatedModulesAjaxHelper'));
        add_action('wp_ajax_bitforms_zbigin_refresh_fields', array(__CLASS__, 'getFields'));
        add_action('wp_ajax_bitforms_zbigin_refresh_tags', array(__CLASS__, 'getTagList'));
        add_action('wp_ajax_bitforms_zbigin_refresh_users', array(__CLASS__, 'getUsers'));
    }
            ...
            if (!is_wp_error($modulesMetaResponse) && (empty($modulesMetaResponse->status) || (!empty($modulesMetaResponse->status) && $modulesMetaResponse->status !== 'error'))) {
                ...
                foreach ($retriveModuleData as $module) {
                    if (!in_array($module->api_name, ['Activities', 'Social', 'Associated_Products', 'Notes', 'Attachments'])) {
                        $allModules[$module->plural_label] = (object) array(
        ...
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce($_REQUEST['_ajax_nonce'], 'bitforms_save')) {
            ...
            foreach ($allModules as $module) {
                if ($module->api_name !== $queryParams->module) {
                    $relatedModules[$module->plural_label] = (object) array(
                        "api_name" => $module->api_name,
                        "plural_label" => $module->plural_label
                    );
                }
        ...
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce($_REQUEST['_ajax_nonce'], 'bitforms_save')) {
            ...
            if (!is_wp_error($fieldsMetaResponse) && (empty($fieldsMetaResponse->status) || (!empty($fieldsMetaResponse->status) && $fieldsMetaResponse->status !== 'error'))) {
                ...
                foreach ($retriveFieldsData as $field) {
                    $fields[$field->api_name] = (object) array(
            ...
            if (!is_wp_error($usersMetaResponse)) {
                ...
                if (count($users) > 0) {
                    ...
                    foreach ($users as $user) {
                        $allUsers[$user->full_name] = (object) array(
                            'userId' => $user->id,
    ...
    public function execute(IntegrationHandler $integrationHandler, $integrationData, $fieldValues, $entryID, $logID)
    {
        ...
        if (
            ...
            $error = new WP_Error('REQ_FIELD_EMPTY', __('module, fields are required for zoho bigin api', 'bitform'));
