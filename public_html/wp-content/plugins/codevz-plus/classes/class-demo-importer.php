...
class Codevz_Demo_Importer {
	...
	public static function replace_demo_link( $data = '', $encoded = false, $only_urls = false, $custom_folder = '' ) {
		...
		$api = apply_filters( 'xtra_config_api', Codevz_Plus::$api );
	...
	public static function import_process( $args = false ) {
		...
		foreach( $args['features'] as $i => $key ) {
			...
			if ( $key === 'images' ) {
				...
				do_action( 'xtra/importer/after_import_theme_options', $options );
				...
				if ( $widgets ) {
					...
					do_action( 'xtra/importer/after_import_widgets' );
				...
				do_action( 'xtra/importer/after_import_content' );
				...
				do_action( 'xtra/importer/after_import_slider' );
