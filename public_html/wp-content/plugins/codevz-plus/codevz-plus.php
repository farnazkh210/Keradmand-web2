...
class Codevz_Plus {
	...
	protected function __construct() {
		...
		// Share icons.
		add_action( 'xtra/share', [ $this, 'share' ] );
	...
	public function plugin_action_links( $links ) {
		...
		$links['codevz_dashboard'] = sprintf( '<a href="%1$s" target="_blank" class="codevz-plus-dashboard">%2$s</a>', get_admin_url() . 'admin.php?page=xtra-activation', esc_html__( 'Dashboard', 'codevz' ) );

		if ( self::is_free() ) {

			$links['codevz_go_pro'] = sprintf( '<a href="%1$s" target="_blank" class="codevz-plus-gopro">%2$s</a>', esc_url( get_admin_url() ) . 'admin.php?page=xtra-activation', esc_html__( 'Activate', 'codevz' ) );
	...
	public static function lazyload( $c ) {
		...
		if ( self::option( 'lazyload' ) == 'true' ) {
			...
			foreach ( $matches as $key ) {
				if ( isset( $key[0] ) && ! self::contains( $key[0], [ 'data:image', 'data-thumb', 'data-bg', 'data-ww=', 'rev-slide', 'xmlns' ] ) ) {
	...
	public static function pro_badge( $link = true ) {
		...
		if ( $link ) {
			...
			return '<a href="' . esc_url( get_admin_url() ) . 'admin.php?page=xtra-activation" target="_blank" class="xtra-pro"><span">' . esc_html__( 'PRO', 'xtra' ) . '</span></a>';
