	...
	class Codevz_Core_Theme {
		...
		public static function wp_enqueue_scripts() {
			...
			if ( ! isset( $_POST['vc_inline'] ) ) {
				...
				// Woocommerce.
				if ( function_exists( 'is_woocommerce' ) && ! wp_script_is( 'codevz-woocommerce' ) && ! wp_script_is( 'xtra-woocommerce' ) ) {
			...
			if ( $is_404 ) {
				...
				if ( have_posts() ) {
					...
					if ( $cpt !== 'elementor_library' && $page !== 'page' && ! $blank && ( $title === '1' || $title === '2' || $title === '8' ) ) {
						...
						echo '<span class="xtra-post-title-date"><a href="' . esc_url( self::$home_url . get_post_time( 'Y/m/d' ) ) . '"><time datetime="' . esc_attr( get_the_date( 'c' ) ) . '"><i class="far fa-clock mr8" aria-hidden="true"></i>' . esc_html( get_the_date() ) . '</time></a></span>';
